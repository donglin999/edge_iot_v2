...
chmod +x "$OUT/load-and-up.sh" "$OUT/validate-env.sh" "$OUT/import-and-start.sh" "$OUT/status.sh"

echo ""
echo "==> 完成。安装包目录："
echo "    $OUT"
echo "    ├── images.tar ($(du -h "$OUT/images.tar" | cut -f1)) + .sha256"
echo "    ├── docker-compose.yml / .env.example / load-and-up.sh / validate-env.sh / README.md"
echo "    └── manifest.txt"
