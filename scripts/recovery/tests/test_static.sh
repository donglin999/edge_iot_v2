...
grep -q 'enable_ipv6: false' "$RECOVERY_DIR/docker-compose.m0-ci.yml"
grep -q 'com.docker.network.bridge.gateway_mode_ipv4: isolated' "$RECOVERY_DIR/docker-compose.m0-ci.yml"
if grep -Eq '^[[:space:]]+(ports|published|host_ip):' "$RECOVERY_DIR/docker-compose.m0-ci.yml"; then
  echo "internal recovery stack must not publish host ports" >&2
  exit 1
fi
...
grep -q 'docker network create --driver bridge --internal --ipv6=false' "$RECOVERY_DIR/m0_ci_drill.sh"
grep -q -- '--opt com.docker.network.bridge.gateway_mode_ipv4=isolated' "$RECOVERY_DIR/m0_ci_drill.sh"
...
grep -q 'python3 "$SAFETY" app-network' "$RECOVERY_DIR/m0_ci_drill.sh"
grep -q 'python3 "$SAFETY" recovery-scripts --root "$SCRIPT_DIR"' "$RECOVERY_DIR/m0_ci_drill.sh"
grep -q 'python /recovery/stack_smoke.py --base-url http://web' "$RECOVERY_DIR/m0_ci_drill.sh"
