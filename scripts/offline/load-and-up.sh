...
ENV_ARGS=(); ENV_FILE=""; UP_ARGS=(); LOAD_ONLY=0; PROFILE_ARGS=(--profile ui); MODE="完整模式(带界面)"
while [ $# -gt 0 ]; do
  case "$1" in
    ...
    --env-file) [ "$#" -ge 2 ] || { echo "!! --env-file 缺少文件路径" >&2; exit 2; }
                [ -z "$ENV_FILE" ] || { echo "!! --env-file 只能指定一次" >&2; exit 2; }
                ENV_FILE=$2
                ENV_ARGS+=(--env-file "$2")
                shift 2 ;;
...
if [ "$LOAD_ONLY" -ne 1 ]; then
  ...
  [ -x ./validate-env.sh ] || {
    echo "!! 缺少可执行的 ./validate-env.sh，离线包可能不完整" >&2
    exit 2
  }
  ./validate-env.sh "$ENV_FILE"
  v=$(grep -E '^WEB_PORT=' -- "$ENV_FILE" 2>/dev/null | tail -1 | cut -d= -f2)
  [ -n "${v:-}" ] && WEB_PORT="$v"
  ...
  "${DC[@]}" "${ENV_ARGS[@]}" "${PROFILE_ARGS[@]}" -f docker-compose.yml config --quiet
fi

# redis:7.0.10 自 2026-07-27 起打进 images.tar(现场实测目标机没有该镜像且无网,
# "目标机已有"的假设不成立)。docker load 后统一自检,这里不再前置拦截。
echo "==> 提醒：InfluxDB 由生产环境自行运行，连接目标由受保护 env 文件配置"
...
echo ""
"${DC[@]}" "${ENV_ARGS[@]}" "${PROFILE_ARGS[@]}" -f docker-compose.yml ps
echo ""
if [ ${#PROFILE_ARGS[@]} -gt 0 ]; then
  echo "==> 健康检查：稍候片刻后访问  http://<本机IP>:${WEB_PORT:-80}/   应返回前端页面"
  echo "    配置完成后可切到省资源的采集模式： ${DC[*]} --env-file $ENV_FILE -f docker-compose.yml stop django web"
else
  echo "==> 采集模式已启动（无界面）。采集在 celery-acq 中运行，会自动恢复上次 RUNNING 的会话。"
  echo "    需要配置时开界面： ${DC[*]} --env-file $ENV_FILE --profile ui -f docker-compose.yml up -d"
fi
